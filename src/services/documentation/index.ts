...
const updateDocumentationSchema = z.object({
  teamId: z.string().min(1, '团队ID不能为空').optional(),
  agentId: z.string().min(1, '代理ID不能为空').optional(),
    ...
    try {
      ...
      const [totalDocs, overviewDocs, technicalDocs, designDocs] = await Promise.all([
        ...
        prisma.documentation.count({
          where: { ...whereClause, type: 'overview' },
        }),
        prisma.documentation.count({
          where: { ...whereClause, type: 'technical' },
        }),
        prisma.documentation.count({
          where: { ...whereClause, type: 'design' },
        }),
      ]);
