...
export default async function requirementsRoutes(fastify: FastifyInstance) {
    ...
    // 根据ID获取问题
    fastify.get('/requirement-questions/:id', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { id } = request.params as { id: string };
    ...
    // 更新问题
    fastify.put('/requirement-questions/:id', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { id } = request.params as { id: string };
    ...
    // 删除问题
    fastify.delete('/requirement-questions/:id', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { id } = request.params as { id: string };
