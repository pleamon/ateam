...
export default async function dataStructureRoutes(fastify: FastifyInstance) {
    ...
    // 批量创建数据结构
    fastify.post('/projects/:projectId/data-structures/batch', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { projectId } = request.params as { projectId: string };
            const { dataStructures } = request.body as { dataStructures: any[] };
            const result = await DataStructureService.batchCreateDataStructures(projectId, dataStructures);
            return reply.status(201).send(result);
        } catch (error) {
            return reply.status(400).send({
                success: false,
    ...
    // 获取项目的所有数据结构
    fastify.get('/projects/:projectId/data-structures', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { projectId } = request.params as { projectId: string };
    ...
    // 按表名获取数据结构
    fastify.get('/projects/:projectId/data-structures/table/:tableName', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
