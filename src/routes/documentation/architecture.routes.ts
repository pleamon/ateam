...
export default async function architectureRoutes(fastify: FastifyInstance) {
  ...
  // 获取项目系统架构
  fastify.get(
    '/projects/:projectId/system-architecture',
  ...
  // 获取系统架构的平台架构列表
  fastify.get(
    '/system-architecture/:systemArchitectureId/platforms',
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { systemArchitectureId } = request.params as {
          systemArchitectureId: string;
        };
        const result =
          await ArchitectureService.getSystemPlatformArchitectures(systemArchitectureId);
        return reply.send(result);
      } catch (error) {
        return reply.status(500).send({
          success: false,
  ...
  fastify.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { id } = request.params as { id: string };
        const result = await ArchitectureService.updatePlatformArchitecture(
          id,
          request.body as any,
        );
        return reply.send(result);
