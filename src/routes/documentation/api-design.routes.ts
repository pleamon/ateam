...
export default async function apiDesignRoutes(fastify: FastifyInstance) {
    ...
    // 获取项目的所有API设计
    fastify.get('/projects/:projectId/api-designs', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
    ...
    // 根据平台获取项目的API设计
    fastify.get('/projects/:projectId/api-designs/platform/:platform', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { projectId, platform } = request.params as { projectId: string; platform: string };
