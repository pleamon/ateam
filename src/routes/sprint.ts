...
export default async function sprintRoutes(fastify: FastifyInstance) {
    ...
    // 获取项目的所有Sprint
    fastify.get('/projects/:projectId/sprints', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
    ...
    // 获取当前活跃的Sprint
    fastify.get('/projects/:projectId/sprints/active', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
            const { projectId } = request.params as { projectId: string };
