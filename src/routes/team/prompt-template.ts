...
export default async function promptTemplateRoutes(fastify: FastifyInstance) {
    ...
    // 根据职责获取提示词模板
    fastify.get('/prompt-templates/by-responsibility/:responsibility', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
    ...
    // 初始化默认模板
    fastify.post('/prompt-templates/initialize', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
